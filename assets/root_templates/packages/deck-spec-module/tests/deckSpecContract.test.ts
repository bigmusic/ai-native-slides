...
const fixturePlanPath = path.join(projectRoot, 'tests', 'fixtures', 'deck-spec.fixture.json')
const fixtureSchemaPath = path.join(projectRoot, 'spec', 'deck-spec.schema.json')
...
async function createTempProject(plan?: DeckSpec): Promise<string> {
    const tempRoot = await createProjectTempDir(projectRoot, 'deck-spec-contract')
    const tempProjectDir = path.join(tempRoot, fixtureProjectSlug)
    tempDirs.push(tempRoot)

    await mkdir(path.join(tempProjectDir, 'spec'), { recursive: true })
    await copyFile(fixtureSchemaPath, path.join(tempProjectDir, 'spec', 'deck-spec.schema.json'))
...
async function validatePlan(plan: DeckSpec): Promise<ReturnType<typeof validateDeckSpecDocument>> {
...
describe('deck-spec contract', () => {
    ...
    it('reads the canonical deck-spec through typed accessors', async () => {
        ...
        expect(requireSlideById(deckSpec, 'overview_hero').layout_intent).toBe('hero')
        expect(requirePlainTextAssetContent(deckSpec, 'hero_message')).toContain('validated spec')
        expect(requireBulletListAssetContent(deckSpec, 'review_message_bullets')).toHaveLength(3)
        expect(readTypedDeckSpecSync(tempProjectDir).slides[0]?.slide_id).toBe('overview_hero')
    })
    ...
    it('rejects consumer-side aliases that drift from the canonical contract', async () => {
        ...
        expect(() => assertDeckSpecShape(invalidPlan, 'fixture deck-spec')).toThrow(/slide_id/)
    })
    ...
    it('normalizes system-managed fields without mutating the input', async () => {
        ...
        mutatedPlan.asset_manifest.image_assets[0].output_file_name = 'bad-name.jpg'
        ...
        expect(mutatedPlan.asset_manifest.image_assets[0].output_file_name).toBe('bad-name.jpg')
    })

    it('derives missing text asset labels and strips planner-only text asset fields', async () => {
        const originalPlan = await loadFixturePlan()
        const mutatedPlan = clonePlan(originalPlan) as unknown as {
            asset_manifest: {
                text_assets: Array<Record<string, unknown>>
            }
        }
        delete mutatedPlan.asset_manifest.text_assets[0].asset_label
        mutatedPlan.asset_manifest.text_assets[0].asset_id_prefix = 'hero_badge'

        const normalizedPlan = normalizeSystemManagedFields(mutatedPlan as unknown as DeckSpec, {
            projectSlug: fixtureProjectSlug,
            sourcePrompt: originalPlan.source_prompt,
        })
        const normalizedAsset = normalizedPlan.asset_manifest.text_assets[0] as Record<
            string,
            unknown
        >

        expect(normalizedAsset.asset_label).toBe('Hero Badge')
        expect('asset_id_prefix' in normalizedAsset).toBe(false)
        expect((await validatePlan(normalizedPlan)).ok).toBe(true)
    })
    ...
    it('derives stable output filenames independent of asset_label', async () => {
        ...
        expect(deriveOutputFileName(asset)).toBe('overview_hero__hero_visual__large.jpg')
    })

    it('rejects missing required fields', async () => {
        const plan = clonePlan(await loadFixturePlan()) as unknown as Record<string, unknown>
    ...
    it('rejects duplicate asset ids', async () => {
        const plan = clonePlan(await loadFixturePlan())
        plan.asset_manifest.image_assets[0].asset_id = plan.asset_manifest.text_assets[0].asset_id
    ...
    it('rejects duplicate block ids', async () => {
        const plan = clonePlan(await loadFixturePlan())
        plan.slides[1].content_blocks[0].block_id = plan.slides[0].content_blocks[0].block_id
