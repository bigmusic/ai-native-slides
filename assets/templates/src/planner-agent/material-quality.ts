...
function requiresExplicitAudienceFraming(sourcePrompt: string): boolean {
	return (
		audienceFramingPromptPattern.test(sourcePrompt) ||
		(audienceFramingSlidePattern.test(sourcePrompt) &&
			workflowDetailPromptPattern.test(sourcePrompt))
	);
}

function hasAudienceFramingBeforeWorkflow(plan: DeckSpec): boolean {
	const audienceSlideIndex = plan.slides.findIndex((slide) =>
	...
	if (audienceSlideIndex === -1) {
		return false;
	}

	if (workflowSlideIndex === -1) {
		return true;
	}

	return audienceSlideIndex < workflowSlideIndex;
}
...
export function collectMaterialQualitySignals(
	...
	if (
		requiresExplicitAudienceFraming(sourcePrompt) &&
		!hasAudienceFramingBeforeWorkflow(plan)
	) {
		signals.push({
