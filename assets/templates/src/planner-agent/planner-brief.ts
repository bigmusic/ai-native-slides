...
export function renderPlannerBriefMarkdown(context: PlannerBriefInput): string {
	...
	return [
		...
		`- After writing the candidate, run \`${context.skill_handoff.promotion_command}\` from the project directory.`,
		`- Treat a non-zero exit from \`${context.skill_handoff.promotion_command}\` as a failed planner phase unless the retry policy below explicitly permits one retry.`,
		...
		`- Maximum promotion attempts: ${context.skill_handoff.max_promotion_attempts}`,
		...
		`- If the first promotion fails with a retryable failure kind, copy the current candidate to \`${context.skill_handoff.debug_paths.last_invalid_candidate}\` and write the stderr failure summary to \`${context.skill_handoff.debug_paths.last_invalid_errors}\`.`,
		"- Fix the candidate using only the reported validation errors. Do not change the user prompt or skip the promotion command.",
