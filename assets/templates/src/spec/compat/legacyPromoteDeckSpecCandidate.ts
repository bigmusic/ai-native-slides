...
const legacySpecPromotionDeprecationMessage =
	'Deprecated: `pnpm spec` without `--prompt` is a transition-only compatibility path for legacy candidate promotion. Use `pnpm spec -- --prompt "<prompt>"` for the main workflow.';
...
export type LegacySpecPromotionDependencies = {
...
export async function promoteDeckSpecCandidate(
	projectDir: string,
	dependencies: LegacySpecPromotionDependencies = {},
...
export async function runLegacySpecPromotionCli(
	projectDir: string,
	io: CliIo,
	dependencies: LegacySpecPromotionDependencies = {},
): Promise<number> {
	io.stderr(legacySpecPromotionDeprecationMessage);
