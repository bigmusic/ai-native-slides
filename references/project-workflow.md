...
When authoring the second-stage project code from that canonical spec, consume the published keys exactly as written. In the current contract, slides use `slide_id`, content blocks use `block_type`, and prompt-planned assets are grouped under `asset_manifest.text_assets`, `asset_manifest.image_assets`, and `asset_manifest.shared_assets`. Do not assume alternate consumer-side aliases such as `id`, `type`, or a flat top-level `assets` array. Also do not treat one earlier run's provider-generated `slide_id` or `asset_id` values as stable across reruns of the same prompt; bind authored deck logic to `layout_intent`, block ordering/slots, and each block's explicit asset reference fields instead. That includes both project-scoped `image_asset_id` references and deck-shared `shared_asset_id` references on `block_type === "image"` blocks.
Prefer the shared accessors re-exported through each project's `src/spec/readDeckSpec.ts`, such as `readTypedDeckSpecSync()`, `requireSlideById()`, `requirePlainTextAssetContent()`, and `requireBulletListAssetContent()`, instead of hand-rolling JSON parsing in second-stage authoring code.
