...
- [ ] 2026-03-15 12:24 PDT: started the shared deck-spec black-box module refactor requested after the completion audit. The root template now carries the first `packages/deck-spec-module/` scaffold plus workspace metadata, and the next slices will migrate writer-first planning/validation runtime code into that shared package before stripping project-local implementations down to thin wrappers.
