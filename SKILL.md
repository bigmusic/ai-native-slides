...
- `scripts/repair_deck_workspace.sh`: Repair deck-local dependencies that can be safely automated, such as `pnpm install`, `.venv` creation, and deck-local Python package installation.
...
5. If `ensure` reports only deck-local dependency issues, run `scripts/repair_deck_workspace.sh <deck-workspace>` to auto-fix them. Leave system-level tools and task-specific source files such as `deck.js` to the user or Codex.
